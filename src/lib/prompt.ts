...
import type { GenerateRequest, CandidateName } from "@/types";
...
export function buildGenerateUserPrompt(req: GenerateRequest): string {
  ...
  // 八字
  if (req.useBazi && req.birthDateTime) {
    parts.push(`\n## 八字信息`);
    parts.push(`出生时间：${req.birthDateTime}（${req.calendarType === "lunar" ? "农历" : "公历"}）`);
    if (req.birthPlace) parts.push(`出生地：${req.birthPlace}`);
    parts.push(
      `请根据八字排盘分析五行，在取名时补益喜用神。请先给出八字/五行分析摘要、四柱干支与五行、五行数量分布、喜用神，再给出候选名字。`,
    );
  }
...
如果提供了出生时间，第一行输出八字分析：
{"summary":"八字五行分析摘要","pillars":{"year":{"gan":"甲","zhi":"子","ganWuxing":"wood","zhiWuxing":"water"},"month":{...},"day":{...},"hour":{...}},"distribution":{"wood":2,"fire":1,"earth":1,"metal":1,"water":3},"xiyongshen":["water","wood"]}

然后每行输出一个名字对象：
{"fullName":"姓+名","givenName":"名","pinyin":"拼音带声调","score":88,"scores":{"wuxing":85,"phonetic":90,"glyph":88,"meaning":92,"poetry":80},"styleProfile":{"classical":30,"modern":20,"literary":80,"majestic":20,"elegant":40,"cute":10,"neutral":20},"meaning":"20字内寓意摘要","recommendation":"20字推荐理由"}

要求：
- 如果没有提供出生时间，不要输出八字那行。
- distribution 中五行为 0-8 的整数。
- styleProfile 中各风格分值为 0-100 的整数，体现该名字在不同风格上的倾向，最高分风格应贴近用户指定的基调。
