...
export function buildGenerateUserPrompt(req: GenerateRequest): string {
  ...
  // 风格
  parts.push(`\n## 风格要求`);
  parts.push(`主风格：${STYLE_LABELS[req.style] ?? req.style}`);
  if (req.styleWeights) {
    const weights = Object.entries(req.styleWeights)
      ...
      .join("、");
    if (weights) parts.push(`风格权重参考：${weights}`);
  }
...
然后每行输出一个名字对象：
{"fullName":"姓+名","givenName":"名","pinyin":"拼音带声调","score":88,"scores":{"wuxing":85,"phonetic":90,"glyph":88,"meaning":92,"poetry":80},"meaning":"20字内寓意摘要","recommendation":"20字推荐理由"}
